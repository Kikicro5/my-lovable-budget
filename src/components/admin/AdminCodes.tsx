...
import { Loader2, Plus, Download, Copy, Key, Users, Calendar, Hash } from 'lucide-react';
...
const AdminCodes = () => {
  ...
  const fetchCodes = useCallback(async () => {
    try {
      const { data, error } = await supabase.functions.invoke('admin-codes', { method: 'GET' });
              ...
              <div key={code.id} className="border border-border rounded-lg p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <code className="text-sm font-mono bg-muted px-2 py-0.5 rounded font-semibold">{code.code}</code>
                ...
                <div className="flex items-center gap-4 text-xs text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{new Date(code.expires_at).toLocaleDateString()}</span>
                </div>
                {code.note && <p className="text-xs text-muted-foreground italic">{code.note}</p>}
